...
for sig in tracker:
    ...
    for e in sig.get("etfs", []):
        ...
        if data:
            exit_p = None
            for d in data:
                if d["date"] >= sig.get("exit_date", ""):
                    exit_p = d["c"]; break
            if exit_p is None and data: exit_p = data[-1]["c"]
            if exit_p and e.get("entry_price"):
                total += (exit_p - e["entry_price"]) / e["entry_price"] * 100
                count += 1
    pnl_str = f"{total/count:+.2f}%" if count > 0 else "N/A"
